export const Section = ({title, content, counter, bg, showAside}: any) => {
    const backgroundColor = bg === 'gray' ? 'bg-light' : 'bg-white';
    const contentJustify = showAside ? 'justify-content-between' : 'justify-content-center';

    return (
        <section className={`row p-4 ${backgroundColor}`} style={{maxWidth: "1320px", margin: "auto"}}>
